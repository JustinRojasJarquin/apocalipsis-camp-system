...
const validateNombre = (nombre: string | undefined, required: boolean) => {
  ...
  if (!trimmed) {
    throw new Error("El nombre del estado no puede estar vacio");
  }

  if (trimmed.length < 3) {
    throw new Error("El nombre del estado debe tener al menos 3 caracteres");
  }

  if (trimmed.length > 100) {
    throw new Error("El nombre del estado no puede exceder 100 caracteres");
  }
};
...
export const validateCreateEstadoPersona = (data: CreateEstadoPersonaDTO) => {
  validateNombre(data.nombre, true);
  validateDescripcion(data.descripcion);
};

export const validateUpdateEstadoPersona = (data: UpdateEstadoPersonaDTO) => {
